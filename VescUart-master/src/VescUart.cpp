...
bool VescUart::processReadPacket(uint8_t * message) {
	...
	switch (packetId){
		...
		case COMM_GET_VALUES: // Structure defined here: https://github.com/vedderb/bldc/blob/43c3bbaf91f5052a35b75c2ff17b5fe99fad94d1/commands.c#L164
			...
			data.avgID				= buffer_get_float32(message, 100.0, &index);
			data.avgIQ				= buffer_get_float32(message, 100.0, &index);
...
void VescUart::setPosition(float position) {
	return setPosition(position, 0);
}

void VescUart::setPosition(float position, uint8_t canId) {
	int32_t index = 0;
	int payloadSize = (canId == 0 ? 5 : 7);
	uint8_t payload[payloadSize];
	if (canId != 0) {
	...
	buffer_append_int32(payload, (int32_t)(position * 1000000), &index);
	packSendPayload(payload, payloadSize);
}
